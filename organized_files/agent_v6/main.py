...
from agent_v6.llm import create_models, shutdown_ollama
from agent_v6.workflow import WorkflowError, run_agentic_workflow


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Agentic Workflow DOCX AI 에이전트 v6")
    ...
    parser.add_argument("--out-dir", type=Path, default=BASE_DIR / "runs_v6")
    parser.add_argument("--prefix", default="agent_v6")
...
def run_once(args: argparse.Namespace, request: str, input_path: Path | None) -> tuple[Path | None, Path]:
    run_dir, result_path, final_observation = run_agentic_workflow(
        request=request,
...
def run_chat(args: argparse.Namespace) -> int:
    ...
    print("DOCX AI 에이전트 v6 대화형 모드")
    print("명령: /open <파일.docx>, /new, /status, /exit")
    print("구조: Planning -> Routing -> Tool Use -> LLM Evaluation -> Reflection -> Finish")
    if current_docx:
